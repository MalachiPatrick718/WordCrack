...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const q = overwrite
      ? admin.from("puzzles").upsert(payload, { onConflict: "puzzle_date" })
      : admin.from("puzzles").insert(payload);

    const { data, error } = await q.select("id,puzzle_date,cipher_word,letter_sets,theme_hint").single();

    if (error) {
      // Friendly status for common uniqueness violation when overwrite=false.
      const msg = error.message ?? "Database error";
      if (!overwrite && msg.toLowerCase().includes("puzzles_puzzle_date_key")) {
        return json(
          { error: "Puzzle already exists for that date. Re-run with overwrite=true or choose a different puzzle_date." },
